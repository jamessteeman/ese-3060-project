# Modified airbench94.py for AdamW (Fixed for FP16 NaNs)
import os
import sys
import uuid
...
hyp = {
    'opt': {
        'train_epochs': 9.9,
        'batch_size': 1024,
        'lr': 0.01,                 # Tuned for AdamW
        'weight_decay': 0.02,        # Tuned for AdamW
        'label_smoothing': 0.1,
...
def main(run):
    ...
    # SPLIT PARAMS: Weight Decay for weights, No Decay for biases/BN
    decay_params = []
    no_decay_params = []
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        # Biases and BN weights (often 1D) usually don't get weight decay in AdamW
        if param.ndim <= 1 or name.endswith(".bias"):
            no_decay_params.append(param)
        else:
            decay_params.append(param)

    # OPTIMIZER: AdamW with eps=1e-4 to support FP16
    optimizer = torch.optim.AdamW([
        {'params': decay_params, 'weight_decay': wd},
        {'params': no_decay_params, 'weight_decay': 0.0}
    ], lr=lr, betas=(0.9, 0.999), eps=1e-4)
    ...
    for epoch in range(ceil(epochs)):
        ...
        # Logging only - not affecting training speed
