...
class Conv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding='same', bias=False, stride=1):
        # Logic to handle padding for strided convolutions
        if stride > 1 and padding == 'same':
            padding = (kernel_size - 1) // 2
            
        super().__init__(in_channels, out_channels, kernel_size=kernel_size, padding=padding, bias=bias, stride=stride)
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, batchnorm_momentum=0.6):
        super().__init__()
        ...
        self.pointwise = Conv(in_channels, out_channels, kernel_size=1, 
                             stride=1, bias=False)
        self.norm = BatchNorm(out_channels, batchnorm_momentum)
...
class ConvGroup(nn.Module):
    def __init__(self, channels_in, channels_out, batchnorm_momentum, use_depthwise=False):
        super().__init__()
        # Use depthwise separable or regular conv
        if use_depthwise:
            self.conv1 = DepthwiseSeparableConv(channels_in, channels_out, batchnorm_momentum)
        else:
    ...
    def forward(self, x):
        x = self.conv1(x)
        # Only apply norm1 if not using depthwise (depthwise has its own norm)
...
def make_net():
    ...
    net = nn.Sequential(
        ...
        nn.GELU(),
        ConvGroup(whiten_width,     widths['block1'], batchnorm_momentum, use_depthwise=True),  # Try depthwise
        ConvGroup(widths['block1'], widths['block2'], batchnorm_momentum, use_depthwise=True),  # Try depthwise
        ConvGroup(widths['block2'], widths['block3'], batchnorm_momentum, use_depthwise=True),  # Try depthwise
        nn.MaxPool2d(3),
        Flatten(),
