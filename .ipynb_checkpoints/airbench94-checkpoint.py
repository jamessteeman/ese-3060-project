...
class ConvGroup(nn.Module):
    def __init__(self, channels_in, channels_out, batchnorm_momentum):
        super().__init__()
        # CHANGE 1: Set stride=2 to downsample here
        self.conv1 = Conv(channels_in,  channels_out, stride=2)
        
        # CHANGE 2: Remove the MaxPool layer
...
def main(run):
    ...
    # CHANGE: Return accuracy AND the total accumulated time
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------
    # CHANGE THIS NUMBER to run 10, 20, or 100 times
    N_RUNS = 100 
    # ---------------------------------------------------------
    ...
    # [1:] means "Skip index 0, take everything else"
    # If N_RUNS=10, this takes the last 9.
    # If N_RUNS=20, this takes the last 19.
    valid_accs = accs[1:]
    valid_times = times[1:]

    print('-' * 55)
    print(f'Running {N_RUNS} times. Discarding run 0 as warmup.')
    print('-' * 55)
    print('Accuracy    -> Mean: %.4f      Std: %.4f' % (valid_accs.mean(), valid_accs.std()))
    print('Total Time  -> Mean: %.4f s     Std: %.4f s' % (valid_times.mean(), valid_times.std()))
    print('-' * 55)

    log = {'code': code, 'accs': accs, 'total_times': times}
